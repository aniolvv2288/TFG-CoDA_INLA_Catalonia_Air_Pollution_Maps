...
#                   LATITUD, LONGITUD) %>%
#   dplyr::summarise(Total_basal = mean(Total_basal, na.rm = TRUE), .groups = "drop")
...
d$station_id <- as.integer(as.numeric(as.factor(d$`NOM ESTACIO`)))
...
stk.e <- inla.stack(
  ...
  A = list(1, A),
  effects = list(
    data.frame(b0 = rep(1, nrow(d)), station_id = d$station_id),
    s = indexs
  )
)
...
stk.p <- inla.stack(
  ...
  A = list(1, Ap),
  effects = list(
    data.frame(b0 = rep(1, nrow(dp)), station_id = NA), # no tenim estació en predicció
    s = indexs
  )
)
...
# Criteri	Amb iid estació	Sense iid estació	Millor si…
# DIC	3096.7	3112.5	Més baix → millor → guanya el model amb iid
# WAIC	3144.3	3148.2	Més baix → millor → guanya el model amb iid
# Marginal log-likelihood	179.4	186.2	Més alt → millor → guanya el model sense iid
# Complexitat (p.eff)	39.8	42.5	El model amb iid és lleugerament més parsimoniós
