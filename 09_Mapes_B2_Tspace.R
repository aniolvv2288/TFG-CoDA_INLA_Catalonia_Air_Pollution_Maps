...
if (!file.exists("model_mes_any_inla_b2.rds")) {
  resultat <- inla(
    formula,
  ...
  saveRDS(resultat, "model_mes_any_inla_b2.rds")
  
} else {
  resultat <- readRDS("model_mes_any_inla_b2.rds")
}
...
plot(resultat.res$marginals.variance.nominal[[1]], ty = "l", 
     xlab = expression(sigma[randomfield]^2), ylab = "Density")  # Variància del camp espacial
plot(resultat.res$marginals.kappa[[1]], type = "l", 
     xlab = expression(kappa), ylab = "Density")                 # Contra rapidesa a la que cau correlació camp espacial
...
if (!file.exists("val_model_mes_any_inla_b2.rds")) {
  p.res <- inla(
    formula,
  ...
  saveRDS(p.res, "val_model_mes_any_inla_b2.rds")
  
} else {
  p.res <- readRDS("val_model_mes_any_inla_b2.rds")
}
...
if (!file.exists("prediccio_model_inla_mes_any_b2.rds")) {
  ...
  saveRDS(dpm2, "prediccio_model_inla_mes_any_b2.rds")
  
} else {
  
  dpm2 <- readRDS("prediccio_model_inla_mes_any_b2.rds")
}
...
ggplot(m) +
  ...
  labs(
    title = "Component B2 del T-space al febrer (2020–2024)",
    x = "", y = ""
  ) +
  theme_bw() +
  theme(
...
ggplot(m) +
  ...
  labs(
    title = "Component B2 del T-space al setembre (2020–2024)",
    x = "", y = ""
  ) +
  theme_bw() +
  theme(
