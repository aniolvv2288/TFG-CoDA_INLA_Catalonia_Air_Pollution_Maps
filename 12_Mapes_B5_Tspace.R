...
if (!file.exists("model_mes_any_inla_b5.rds")) {
  resultat <- inla(
    formula,
  ...
  saveRDS(resultat, "model_mes_any_inla_b5.rds")
  
} else {
  resultat <- readRDS("model_mes_any_inla_b5.rds")
}
...
if (!file.exists("val_model_mes_any_inla_b5.rds")) {
  p.res <- inla(
    formula,
  ...
  saveRDS(p.res, "val_model_mes_any_inla_b5.rds")
  
} else {
  p.res <- readRDS("val_model_mes_any_inla_b5.rds")
}
...
if (!file.exists("prediccio_model_inla_mes_any_b5.rds")) {
  ...
  saveRDS(dpm2, "prediccio_model_inla_mes_any_b5.rds")
  
} else {
  
  dpm2 <- readRDS("prediccio_model_inla_mes_any_b5.rds")
}
...
ggplot(m) +
  ...
  labs(
    title = "Component B5 del T-space al febrer (2020–2024)",
    x = "", y = ""
  ) +
  theme_bw() +
  theme(
